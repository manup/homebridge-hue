...
module.exports = class HuePlatform extends LibPlatform {
  ...
  foundBridge(host) {
    ...
    this.httpGet(url).then((config) => {
      ...
      if (!context.id || context.id === '0000000000000000') {
        this.warn('ingoring uninitialised bridge %j', context);
        return;
      }
      switch (context.model) {
        case 'BSB001':
        case 'BSB002':
          context.manufacturer = 'Philips';
          break;
        case 'deCONZ':
          context.manufacturer = 'dresden elektronik';
          break;
        default:
          context.manufacturer = '(unknown)';
          break;
      }
      if (this.hueBridges[context.id]) {
        if (context.host !== this.hueBridges[context.id].context.host) {
          this.debug('bridge %s now at %s', context.id, context.host);
          this.hueBridges[context.id].updateHost(context.host);
        }
      } else {
        this.debug('found bridge %s at %s', context.id, context.host);
        this.hueBridges[context.id] = new HueBridge(this, context);
      }
    }).catch((error) => {
      this.error(error);
    });
