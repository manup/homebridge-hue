...
module.exports = class HueBridgeAccessory extends LibAccessory {
  ...
  constructor(platform, context) {
    ...
    context.category = this.Accessory.Categories.BRIDGE;
    this.bridge = new HueBridgeService(this, context);
    this.lights = {};
    this.groups = {};
    this.sensors = {};
    this.schedules = {};
    ...
    this.on('identify', this.onIdentify.bind(this));
  }
  ...
  configure() {
    ...
    switch (this.manufacturer) {
      ...
      case 'dresden elektronik':
        switch (this.model) {
          case 'deCONZ':
            if (this.context.apiversion !== '1.0.1') {
              this.warning('api version %s', this.context.apiversion);
            }
  ...
  onIdentify() {
    this.info(
      '%s %s bridge, api v%s',
      this.manufacturer, this.model, this.context.apiversion
    );
  }
  ...
  updateHost(host) {
    ...
    this.url = this.context.username ?
      this.msg('http://%s/api/%s', this.context.host, this.context.username) :
      this.msg('http://%s/api', this.context.host);
    this.onIdentify();
  }
  ...
  getUser() {
    ...
    if (!this.url) {
      ...
      this.context.parallelRequests = this.model === 'BSB001' ? 3 : 10;
