...
module.exports = class HueBridge extends LibAccessory {
  ...
  heartbeat(beat) {
    if (beat % this.bridge.heartrate !== 0) {
      return;
    }
    this.getUser()
  ...
  updateHost(host) {
    ...
    this.url = this.context.username ?
      util.format('http://%s/api/%s', this.context.host, this.context.username) :
  ...
  getUser() {
    ...
    if (!this.url) {
      ...
      this.bridge.heartrate = 30;
      this.context.parallelRequests = this.context.model === 'BSB001' ? 3 : 10;
      ...
      if (err) {
        ...
        if (responseBody === '' && response.statusCode !== 200) {
          ...
          for (const id in responseBody) {
            ...
            if (e) {
              errorMessages.push(
                util.format('bridge error %d: %s', e.type, e.description)
              );
